...
public struct PrefsValue<T>
{
	...
	public PrefsValue(string key, T defaultValue, bool autoSave, PrefsType prefsType)
	{
		...
		else
		{
			...
			Leyoutech.Utility.DebugUtility.LogError("Prefs", $"ValueItem not support value type({type})");
		}
			...
			case PrefsType.Editor:
				switch (m_ValueType)
				{
					...
					case ValueType.Int:
						...
						break;
					default:
						Debug.LogError(string.Format("Unresolved Save ValueType({0})", m_ValueType));
						break;
				}
				break;
		...
		switch (m_PrefsType)
		{
			case PrefsType.Editor:
				switch (m_ValueType)
				{
					...
					default:
						Debug.LogError(string.Format("Unresolved Load ValueType({0})", m_ValueType));
						break;
				}
				break;
			case PrefsType.Player:
